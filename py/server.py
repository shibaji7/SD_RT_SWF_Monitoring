...
class DataMemory(object):
    ...
    def __init__(self, rads=["wal", "bks", "fhe", "fhw", "cve", "cvw", "kap", "gbr", "sas", "pgr"],
                 Ts=dt.datetime(2015,3,11,16), Te=dt.datetime(2015,3,11,17)):
        self.rads = rads
        self.Ts = Ts
        self.Te = Te
        self.frames = {}
...
class InfoRequestHandler(object):
    ...
    def get_color_by_radpos(self):
        c = {"r": 255, "g": 0, "b": 0}
        if self.rad in ["gbr", "pgr", "sas", "kap"]: c = {"r": 0, "g": 0, "b": 255}
        return c
...
class PhaseRequestHandler(object):
    ...
    def parse_radar_phase(self):
        ...
        self.resp["rad"] = self.rad
        self.resp["pe"] = pe
        self.resp["alpha"] = 1./(1. + np.exp(pe-1))
        self.resp["phase"] = ph
        return
