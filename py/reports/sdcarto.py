...
class SDCarto(GeoAxes):
    ...
    def overlay_radar(self, rad, tx=cartopy.crs.PlateCarree(), zorder=2, markerColor="darkblue", markerSize=8, fontSize=5, font_color="k",
            xOffset=None, yOffset=-2.5, annotate=True):
        ...
        if annotate:
            if rad in nearby_rad[0]: xOff, ha = 2.5 if not xOffset else xOffset, 0
            elif rad in nearby_rad[1]: xOff, ha = -2.5 if not xOffset else xOffset, 1
            ...
            self.text(x, y, rad.upper(), ha="center", va="center", transform=self.projection,
                     fontdict={"color":markerColor, "size":fontSize})
        return
    
    def overlay_fov(self, rad=None, tx=cartopy.crs.PlateCarree(), maxGate=40, rangeLimits=None, beamLimits=None,
    ...
    def enum(self, add_date=True, add_coord=True, dtype=None):
        if add_coord: self.text(-0.02, 0.99, "Coords: %s"%self.coords, horizontalalignment="right",
                                verticalalignment="top", transform=self.transAxes, rotation=90, 
                                fontdict={"weight":"bold"})
        if dtype is not None: self.text(0.01, 1.05, dtype.upper(), ha="left", va="center", 
                                        ransform=self.transAxes, fontdict={"color":"r"})
        if add_date: self.text(0.99, 1.05, self.plot_date.strftime("%Y-%m-%d %H:%M") + " UT",
                               horizontalalignment="right", verticalalignment="center", 
                               transform=self.transAxes, fontdict={"weight":"bold"})
        return
    ...
    def data_lay(self, df, rf, p_name, to, fm, idx=None, gflg_mask=None):
        ...
        if len(o) > 0:
            ...
            Xb, Yg, Px = get_gridded_parameters(o, xparam="bmnum", yparam="slist", zparam=p_name, rounding=True)
            Xb, Yg = Xb.astype(int), Yg.astype(int)
            lons, lats = lons[Xb.ravel(), Yg.ravel()].reshape(Xb.shape),\
                        lats[Xb.ravel(), Yg.ravel()].reshape(Xb.shape)
    ...
    def _add_colorbar(self, bounds, colormap, label=""):
        ...
        cpos = [pos.x1 + 0.035, pos.y0 + 0.01*pos.height,
...
class FanPlots(object):
    ...
    def __init__(self, figsize=(3.5,3.5), nrows=1, ncols=1, dpi=150, coords="geo"):
    ...
    def plot_fov(self, date, rads):
        """ Plot data - overlay on top of map """
        ax = self._add_axis(date)
        for rad in rads:
            ax.overlay_radar(rad)
            ax.overlay_fov(rad)
            ax.overlay_fov(rad, beamLimits=[7,8], fovColor="r")
        ax.enum()
        ax.set_extent([-120, -70, 30, 60], crs=ccrs.PlateCarree())
        return ax
    
    def plot_rad_data(self, date, rad, add_colorbar=False):
        """ Plot data - overlay on top of map """
        ax = self._add_axis(date)
        ax.overlay_radar(rad)
        ax.overlay_fov(rad, maxGate=100)
        fdata = gfd.FetchData( rad, [date, date+dt.timedelta(minutes=2)] )
        beams, _ = fdata.fetch_data()
        df = fdata.convert_to_pandas(beams)
        ax.overlay_radar_data(rad, df, self.orthographic, self.geodetic, gflg_mask=None, 
                              add_colorbar=add_colorbar)
        return ax
    
def create3X3summaryplot(dates, rads):
    fp = FanPlots((6,5), nrows=3, ncols=3, dpi=180)
    for i, r in enumerate(rads):
        for j, d in enumerate(dates): 
            add_colorbar = True if (i==2 and j==2) else False
            ax = fp.plot_rad_data(d, r, add_colorbar=add_colorbar)
            ax.enum()
    return