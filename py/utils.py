...
def read_events():
    events = pd.read_csv(
        f"assets/data/FL.csv",
        parse_dates=["date", "event_starttime", "event_peaktime", "event_endtime"],
    )
    color_codes = pd.read_csv(f"assets/data/Colors.csv", parse_dates=["date"])
...
def select_event_by_color_code_date(events, color_codes, date):
    ...
    if len(color_code) > 0:
        clas = "C"
        if color_code.color.iloc[0] == "red":
            clas = "X"
        if color_code.color.iloc[0] == "yellow":
            clas = "M"
        event = event[(event.date == date) & (event.flare_class == clas)]
        start_time, end_time = (
...
def create_date_list(date, whole_month):
    ...
    dates = (
        [date]
        if nod == 1
        else [date.replace(day=1) + dt.timedelta(d) for d in range(nod)]
    )
    return dates
