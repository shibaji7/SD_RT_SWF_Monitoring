...
class PhaseDetector(object):
    ...
    def fetch_echoes_dataset(self):
        ...
        scan_min_time = self.scan_time / 60.0
        dat["min_of_day"] = dat.time.apply(
            lambda x: scan_min_time * int((x.hour * 60 + x.minute) / scan_min_time)
        )
    ...
    def train_AR(self, rt=False):
        ...
        ar_max_order = int(self.train_hours * 3600.0 / self.scan_time)
        mfname = self.save_train_file + "ar.e%s.d%d.model.pickle" % (
            self.prev_date.strftime("%d%b%y"),
            self.train_days,
        )
        dfname = self.save_train_file + "ar.e%s.d%d.model.csv" % (
            self.prev_date.strftime("%d%b%y"),
            self.train_days,
        )
        if rt and os.path.exists(mfname):
            os.system("rm " + mfname)
        if rt and os.path.exists(dfname):
            os.system("rm " + dfname)
        ...
        if not os.path.exists(mfname):
            ...
            self.model["ar.resp"] = sm.load(mfname)
        return
    ...
    def forecast(self, L=1, model_name="ar"):
        ...
        if not key in self.model.keys():
            self.train_AR()
        model = self.model["%s.resp" % model_name]
        if model_name == "ar":
            pred = model.forecast(L)
        return pred
...
class Detectors(object):
    ...
    def preset(self, r):
        if r not in self.predictors.keys():
            self.predictors[r] = PhaseDetector(r)
        if r not in self.phases.keys():
            self.phases[r] = "p-SWF"
        if r not in self.echoes_dev.keys():
            self.echoes_dev[r] = 0.0
        return
    ...
    def detect_SWF_phase(self, r, o, dn):
        ...
        if pe <= 0.5:
            opd.update_AR(d)
        if (pe > 0.5 and pe <= 0.9) and self.phases[r] == "p-SWF":
            self.phases[r] = "o-SWF"
        if pe > 0.9 and self.phases[r] in ["o-SWF", "p-SWF"]:
            self.phases[r] = "b-SWF"
        if (pe > 0.5 and pe <= 0.9) and self.phases[r] in ["o-SWF", "b-SWF"]:
            self.phases[r] = "r-SWF"
        if pe <= 0.1 and self.phases[r] in ["b-SWF", "r-SWF", "o-SWF"]:
            self.phases[r] = "p-SWF"
    ...
    def print_summary(self, dn):
        for r in self.phases.keys():
            logger.info(
                f"Infered procs {dn}: {r.upper()}({self.phases[r]}, {self.echoes_dev[r]})"
            )
        return
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "-r", "--rad", default="bks", help="SuperDARN radar code (default bks)"
    )
    p.add_argument(
        "-t", "--retrain", action="store_true", help="Retrain the model (default False)"
    )
    args = p.parse_args()
